...
BASIS_FILT_DIR <- file.path(BNEALE_DIR,'as_basis_ic_tmp')
SNP_MANIFEST_FILE <-'/home/ob219/share/as_basis/ichip/snp_manifest/ichip_all.tab'
BB_BASIS_LU_FILE <- '/home/ob219/share/as_basis/ichip/support/sept_bb_ic_var_man.RDS'
OUT_DIR <- '/home/ob219/share/as_basis/ichip/bb_projections/shrink_2018/'
...
if(FALSE){
  ...
  #cmds <- sapply(c(165,159,78,167,116,57,166),function(i){
    sprintf("Rscript /home/ob219/git/basis_paper/ichip/UK_BIOBANK/process_bb_self_reported_disease_august_2018_ic.R -i %d",i)
  })
  write(cmds,file="~/tmp/qstuff/ic_bb_disease_proj_2018.txt")
}
...
if(FALSE){
  ...
  bb_man <- M[!is.na(varid) & !duplicated(pid),]
  alleles <- data.table(al.x = paste(M$ref_a1,M$ref_a2,sep='/'),al.y=paste(M$bb_ref,M$bb_alt,sep='/'))
  ## to make quick
  ...
  saveRDS(bb_man[,.(pid,lookup,flip)],BB_BASIS_LU_FILE)
}
lu <- readRDS(BB_BASIS_LU_FILE)
lu[,keep:=lookup %>% gsub("\\_",':',.)]
...
if(!file.exists(file.path(BASIS_FILT_DIR,filt.fname))){
  DT<-fread(sprintf("zcat %s",file.path(BNEALE_DIR,med$ofile[i])))[variant %in% lu$keep, ]
...
## flip or for those that need it
out[pid %in% lu[flip==TRUE,]$pid,or:=1/or]
