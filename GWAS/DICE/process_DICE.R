...
if(!TEST){
  ...
  if (is.null(args$fname) | is.null(args$dir)){
...
if(FALSE){
  ...
  DATA_DIR <- '/home/ob219/share/as_basis/GWAS/sum_stats/DICE'
  dirs <- list.dirs(path=DATA_DIR,recursive=FALSE)
  for(d in dirs){
    files <- list.files(path=d,pattern="*.tab",full.names=FALSE)
    ensgs <- basename(files) %>% gsub("\\.tab","",.)
    CHUNK_SIZE <- 100
    gchunk <- split(ensgs, ceiling(seq_along(ensgs)/CHUNK_SIZE))
    for(i in seq_along(gchunk)){
      fname <- file.path(d,sprintf("gene_%d.txt",i))
      write(gchunk[[i]],file=fname)
    }
    files <- list.files(path=d,pattern="*.txt",full.names=TRUE)
