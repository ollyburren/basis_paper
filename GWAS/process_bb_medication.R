...
if(FALSE){
  ...
  med <- pheno[grepl("20003\\_",Phenotype.Code) & Sex=='both_sexes',]
  cmds <- sapply(1:nrow(med),function(i){
    sprintf("Rscript /home/ob219/git/as_basis/R/Individual_projection/process_bb_medication.R -i %d",i)
  })
...
odir <- '/home/ob219/rds/hpc-work/as_basis/bb/summary_stats_20180731/medications/'
med[,new.cmd:=sprintf("wget %s -O %s%s",Dropbox.File,odir,ofile)]
med[,phe:=make.names(Phenotype.Description) %>% gsub("Treatment.medication.code..","",.)]
...
OUT.DIR <- '/home/ob219/rds/hpc-work/as_basis/bb/summary_stats_20180731/medications/june_10k/'
