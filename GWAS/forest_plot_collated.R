...
pdf(file="~/tmp/everything_forest_2019.pdf",paper="a4",width=8,height=14,onefile=TRUE)
lapply(paste('PC',1:10,sep=''),function(pc){
  if(!pc %in% c('PC10','PC111') ){
    forest_plot(res.DT,pc=pc)
  }else{
    forest_plot(res.DT,pc=pc,theme=theme(panel.grid.major.y=element_line(colour='lightgrey',linetype=3),axis.text.y=element_text(size=8)))
  }
})
dev.off()
