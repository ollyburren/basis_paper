...
myo.DT <- fread("~/share/Data/GWAS-summary/MYOGEN/imputed_summary_stats/gwas_dmjdmpm_new_ssimp.meta.txt")
## check reimputed
...
#myo.DT[pid %in% myo.DT[duplicated(pid),],]
myo.DT[,id:=1:.N]
...
#M[,seb:=1/sqrt(2 * N.imp * maf * (1-maf))]
## compute odds ratio
#M[,or_imp:=exp(z_imp * seb)]
n_cases <- 1711
n_controls <- 4724
...
pfile <- file.path(SRC_OUT_DIR,sprintf("%s_source.RDS",'myositis_myogen_ssimp'))
...
tmp[,trait:= 'myositis_myogen_ssimp']
...
saveRDS(all.proj,file='/home/ob219/share/as_basis/GWAS/myogen_myositis/myogen_myositis_ssimp_0619.RDS')
...
for (trait in c('jdm','pm','dm')){
  ...
  stop("Something wrong basis and projection matrix don't match")
  all.proj <- predict(pc.emp,newdata=mat.emp)
  saveRDS(all.proj,file=sprintf('/home/ob219/share/as_basis/GWAS/myogen_myositis/%s_myositis_0619.RDS',trait))
}
