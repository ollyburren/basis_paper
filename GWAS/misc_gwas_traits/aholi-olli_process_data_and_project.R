...
OUT_FILE <- "/home/ob219/share/as_basis/GWAS/ahola-olli_cytokine/projections/ck_13_traits_0919.RDS"
...
res <- mclapply(files,function(f){
  ...
  M <- merge(DT[,.(trait=trait,pid,a1=toupper(OtherAllele),a2=toupper(EffectAllele),beta=Effect,seb=StdErr,p.value=P.value)],man.DT,by='pid',all.y=TRUE)
  M <- M[!duplicated(pid),]
  idx <- which(is.na(M$beta))
  sprintf("%d missing",length(idx)) %>% message
  if(length(idx)!=0)
    M[idx,c('trait','a1','a2','beta'):=list(trait,ref_a1,ref_a2,beta=0)]
  ## align alleles
  ...
  saveRDS(tmp[,.(pid,beta,seb,p.value,ws_emp_shrinkage)],file=pfile)
