...
res.DT <- all.DT[category %in% c('myogen','bb_disease'),]
...
at <- at[!at %in% c('myogen')]
library(RColorBrewer)
cols<-brewer.pal(length(talk.DT$category %>% unique)+1, 'Dark2')
names(cols) <- c('myogen',at,'basis')
cols['myogen'] <- 'deeppink2'
cols['basis'] <- '#7c0799'
...
forest_plot_focal_merge <- function(proj.dat,basis.dat=basis.DT,pc,focal,title,cat_levels,fdr_thresh=0.05,theme=NA){
  ...
  scale_alpha_manual(values=c('TRUE'=1,'FALSE'=0.5)) + scale_colour_manual("Category",values=cat_levels,labels=c('Myogen','UKBB SRD','Basis')) +
...
title <- ''
pp1 <- forest_plot_focal_merge(talk.DT,pc=pc,focal=all.traits['myogen'] %>% unlist %>% gsub("^jia_","",.),title=title,cat_levels=cols)
