...
all.DT <- readRDS("/home/ob219/share/as_basis/GWAS/RESULTS/09_04_19_summary_results.RDS")

## FOREST PLOT CODE

library(cowplot)

res.DT <- all.DT[category %in% c('lyons_egpa','bb_disease'),]
res.DT[category=='lyons_egpa',category:='EGPA']
...
forest_plot_focal_merge <- function(proj.dat,basis.dat=basis.DT,pc,focal,title,cat_levels,fdr_thresh=0.05,theme=NA){
  ...
  scale_alpha_manual(values=c('TRUE'=1,'FALSE'=0.5)) + scale_colour_manual("Category",values=cat_levels,labels=c('EGPA','UKBB SRD','Basis')) +
...
title <- ''
pp1 <- forest_plot_focal_merge(talk.DT,pc=pc,focal=all.traits['EGPA'] %>% unlist %>% gsub("^jia_","",.),title=title,cat_levels=cols)
pp1 <- pp1 + theme(legend.position="bottom")
save_plot("~/tmp/fig3_egpa_forest_pc6.pdf",pp1,base_width=7,base_height=5)
