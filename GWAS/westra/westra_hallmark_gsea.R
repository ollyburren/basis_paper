...
res.DT <- readRDS("/home/ob219/rds/rds-cew54-wallace-share/as_basis/GWAS/westra_projections/proj.RDS")
## define a Z score for loading to see if any are significant across pc's
M <- melt(res.DT,id.vars='trait')
M[,c('hugo','probe'):=tstrsplit(trait,':')]
## remove duplicated genes
M <- M[!duplicated(paste(hugo,variable)),]
...
filters = 'external_gene_name', values = M$hugo, mart =ensembl) %>% data.table
genedesc <- genedesc[chromosome_name %in% c(1:22,'X'),]
genedesc <- genedesc[genedesc[ , .I[which.min(entrezgene)], by = 'hugo']$V1]


M <- merge(M,genedesc,by.x='hugo',by.y='external_gene_name')

## take the minimum entrez id where more than one match the same ensembl id
...
if(FALSE){
  ...
  ggtitle(expression("Westra et al."~beta*"s")) +
  geom_hline(yintercept=0,colour="white")
  dev.print(pdf,"~/tmp/westra.pdf")
...
dev.print(pdf,"~/tmp/hallmark_f_westra.pdf")

## do mean
gsea(hm,test.type='t.test',fdr.method="fdr") %>% phwrap(.,main="t Test Hallmark")
dev.print(pdf,"~/tmp/hallmark_t_westra.pdf")
library(xtable)
