...
if(FALSE){
  ...
  m <- merge(foo,uk10,by.x='SNP',by.y='ID')
  ...
  wout <- filt[,.(SNP,pid)]
  saveRDS(wout,"/home/ob219/share/as_basis/GWAS/geneatlas/lookup.RDS")
}
...
filt[,c('beta.log','se.beta.log'):=list(convertORscale(beta,meta.dt$pro[i]),convertORscale(seb,meta.dt$prop[i]))]
SHRINKAGE_FILE <- '/home/ob219/share/as_basis/GWAS/support/ss_shrinkage_gwas_0619.RDS'
sDT <- readRDS(SHRINKAGE_FILE)
stmp<-sDT[,.(pid,ws_emp_shrinkage)]
tmp <- merge(filt,stmp,by='pid',all.y=TRUE)
