...
processGTex <- function(f,out.dir='/home/ob219/rds/rds-cew54-wallace-share/Data/gtex/eur-Whole_Blood-001/as_basis'){
  (load(f))
  ## get liftover file
  ...
  snps <- merge(snps,uk10m,by.x=c('chromosome','POS_37'),by.y=c('CHR','BP'))
...
if(FALSE){
  ...
  cmds <- lapply(gt.files,function(f){
    sprintf("Rscript /home/ob219/git/as_basis/R/Individual_projection/switch_alleles_GTEX_q.R -f %s",f)
  }) %>% do.call('c',.)
  write(cmds,file="~/tmp/qsub/gtex.txt")
}
